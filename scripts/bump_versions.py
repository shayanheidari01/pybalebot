...
API_VERSION_LINE = re.compile(
    r"(Supports `Bale Bot API )[\d.]+( <https://core\.bale\.org/bots/api>`_ )"
)
...
def write_package_meta(api_version: str) -> None:
    path = Path.cwd() / "pybalebot" / "__meta__.py"
...
def main():
    api_version = get_bale_api_version()

    print(f"Bale Bot API version: {api_version}")
