...
test-run-services:
	@#docker-compose -f tests/docker-compose.yml -p pybalebot3-dev up -d

.PHONY: test
test: test-run-services
	pytest --cov=pybalebot --cov-config .coveragerc tests/ --redis $(redis_connection)

.PHONY: test-coverage
test-coverage: test-run-services
	mkdir -p $(reports_dir)/tests/
	pytest --cov=pybalebot --cov-config .coveragerc --html=$(reports_dir)/tests/index.html tests/ --redis $(redis_connection)
...
docs-serve:
	hatch run docs:sphinx-autobuild --watch pybalebot/ --watch CHANGELOG.rst --watch README.rst docs/ docs/_build/ $(OPTS)
.PHONY: docs-serve
